...
import { trafficRouter } from '#router/traffic.router.ts';
...
app.get('/sse/traffic', trafficRouter);
...
app.get(
  '/traffic',
  serveStatic({ path: './dist/traffic-dashboard/index.html' }),
);
...
const server = serve(
  ...
  (info) => {
    ...
    console.table({
      ...
      Traffic: { URL: `http://localhost:${info.port}/traffic-dashboard` },
    });
